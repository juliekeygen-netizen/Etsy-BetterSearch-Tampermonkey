...
test('load order makes atomic integration the module-75 fallback at release version', async () => {
  ...
  const primitive = userscript.indexOf('/src/61ab-favorites-atomic-mutations.js?v=0.15.20');
  const hardening = userscript.indexOf('/src/73-favorites-phase5-hardening.js?v=0.15.20');
  const runtimeGuard = userscript.indexOf('/src/74-favorites-phase5-runtime-guard.js?v=0.15.20');
  const integration = userscript.indexOf('/src/74a-favorites-atomic-mutable-rows.js?v=0.15.20');
  const lease = userscript.indexOf('/src/75-favorites-phase5-multitab-lease.js?v=0.15.20');
  assert.ok(primitive >= 0 && hardening > primitive && runtimeGuard > hardening && integration > runtimeGuard && lease > integration);
  assert.match(userscript, /@version\s+0\.15\.20/);
});
