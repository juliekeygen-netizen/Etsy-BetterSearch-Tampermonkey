...
test('local card action slot reserves checkout geometry before hover', () => {
  ...
  assert.match(moduleSource, /opacity:0!important;visibility:hidden!important;pointer-events:none!important/);
  assert.match(moduleSource, /:hover \[data-ebsf-owned-cart-slot="1"\].*opacity:1!important;visibility:visible!important;pointer-events:auto!important/s);
...
test('cart actions prefer a live native Etsy control and never use listing navigation for plain Add to cart', () => {
  ...
  const plainAddBranch = moduleSource.slice(moduleSource.indexOf('const nativeControl = favMatchingNativeCartControl01530'), moduleSource.indexOf('document.addEventListener'));
