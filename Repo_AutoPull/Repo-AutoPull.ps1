...
function Normalize-Configuration {
    ...
    if ($null -ne $Config.postPullCommand) { $normalized.postPullCommand = [string]$Config.postPullCommand }
...
function Save-Configuration {
    ...
    [pscustomobject]@{
        ...
        postPullCommand = [string]$normalized.postPullCommand
...
function Read-Configuration {
    ...
    return Normalize-Configuration $config
}
...
function Run-Configuration {
    ...
    $commandLabel = if ([string]::IsNullOrWhiteSpace($currentCommand)) { "off" } else { $currentCommand }
    $commandInput = Read-Host "Custom command [$commandLabel] (Enter = keep, NONE = clear)"
    if ([string]::IsNullOrWhiteSpace($commandInput)) { $postPullCommand = $currentCommand }
    elseif ($commandInput.Trim() -match '^(?i:none|off)$') { $postPullCommand = "" }
    else { $postPullCommand = $commandInput.Trim() }
...
function Invoke-CustomCommand {
    ...
    $oldPreference = $ErrorActionPreference; Push-Location $RepoPath
    try { $ErrorActionPreference = "Continue"; $raw = @(& $env:ComSpec /d /s /c $Command 2>&1); $exitCode = $LASTEXITCODE }
    finally { $ErrorActionPreference = $oldPreference; Pop-Location }
    $lines = @($raw | ForEach-Object { if ($null -ne $_) { $_.ToString() } } | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
    $detail = if ($lines.Count -gt 0) { $lines[$lines.Count - 1] } else { "" }
    return [pscustomobject]@{ Success = ($exitCode -eq 0); Summary = $(if ($exitCode -eq 0) { "Custom command finished" } else { "Custom command failed" }); Detail = $detail }
}
                    ...
                    elseif (Test-IsAncestor -RepoPath $repoPath -Older $localHash -Newer $remoteHash) {
                        ...
                        if ($afterHash -and $afterHash -ne $localHash) {
                            ...
                            if ($pullResult.ExitCode -ne 0 -or $unmerged.Count -gt 0) {
                                ...
                                if ($postSuccess -and -not [string]::IsNullOrWhiteSpace([string]$config.postPullCommand)) {
                                    Render-Dashboard -RepoPath $repoPath -IntervalInfo $intervalInfo -UpstreamInfo $upstreamInfo -Config $config `
                                        -HeadHash $afterHash -HeadSubject (Get-CommitSubject -RepoPath $repoPath -Hash $afterHash) `
                                        -Status "Running custom command..." -StatusKind "working" -Detail ([string]$config.postPullCommand) `
                                        -LastCheck $lastCheck -NextCheck $null -PullHistory $pullHistory
                                    $commandResult = Invoke-CustomCommand -RepoPath $repoPath -Command ([string]$config.postPullCommand)
                                    if ($commandResult.Success) { $actionParts += "command OK" }
                                    else { $postSuccess = $false; $postFailure = "$($commandResult.Summary): $($commandResult.Detail)"; $actionParts += "command FAILED" }
                                }
