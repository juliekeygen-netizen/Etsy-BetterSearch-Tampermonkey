...
const description = metadataValue(metadata, 'description', 'Better Etsy search and Favorites filtering.');
...
const contentBridge = await readFile(resolve(ROOT, 'extension/content-bridge.js'), 'utf8');
const contentBridgeFinal = await readFile(resolve(ROOT, 'extension/content-bridge-final.js'), 'utf8');
const backgroundCoordinator = await readFile(resolve(ROOT, 'extension/background.js'), 'utf8');
const backgroundWorker = await readFile(resolve(ROOT, 'extension/background-worker.js'), 'utf8');
const backgroundWorkerFinal = await readFile(resolve(ROOT, 'extension/background-worker-final.js'), 'utf8');
...
const popupJs = await readFile(resolve(ROOT, 'extension/popup.js'), 'utf8');
const moduleSources = [];
...
const contentSource = `// Generated by scripts/build.mjs from the Tampermonkey module order.\n// Do not edit dist/content.js directly.\n\n(async () => {\n'use strict';\n\n${platformPrelude.trim()}\n${moduleSources.join('\n')}\n// ===== extension/content-bridge.js =====\n${contentBridge.trim()}\n\n// ===== extension/content-bridge-final.js =====\n${contentBridgeFinal.trim()}\n})().catch((error) => {\n  console.error('Etsy BetterSearch extension failed to initialize.', error);\n});\n`;

// Only worker-safe shared primitives are reused. Mutable catalogue/queue
// ownership is implemented in background-worker.js rather than loading the
// page runtime's historical wrapper stack into a service worker. 61ab supplies
// the current atomic single-row primitive; 61c supplies the raw HTML parser.
const backgroundSharedPaths = [
  'src/61a-favorites-index.js',
  'src/61ab-favorites-atomic-mutations.js',
  'src/61c-favorites-deep-parser.js'
];
const backgroundSharedSources = [];
for (const path of backgroundSharedPaths) {
  const source = await readFile(resolve(ROOT, path), 'utf8');
  backgroundSharedSources.push(`\n// ===== shared background library: ${path} =====\n${source.trim()}\n`);
}
const backgroundSource = `// Generated by scripts/build.mjs.\n// Worker-safe Favorites primitives run against extension-origin IndexedDB.\n\n'use strict';\n${backgroundSharedSources.join('\n')}\n// ===== extension/background-worker.js =====\n${backgroundWorker.trim()}\n\n// ===== extension/background-worker-final.js =====\n${backgroundWorkerFinal.trim()}\n\n// ===== extension/background.js =====\n${backgroundCoordinator.trim()}\n`;
...
if (diagnosticsManifest.background?.service_worker !== 'service-worker.js') throw new Error('Diagnostics manifest must use the service-worker wrapper.');
...
for (const target of ['chrome', 'firefox']) {
  ...
  await Promise.all([
    ...
    writeFile(resolve(targetDir, 'popup.html'), `${popupHtml.trim()}\n`, 'utf8'),
    writeFile(resolve(targetDir, 'popup.css'), `${popupCss.trim()}\n`, 'utf8'),
    writeFile(resolve(targetDir, 'popup.js'), `${popupJs.trim()}\n`, 'utf8'),
    writeFile(resolve(targetDir, 'BUILD_INFO.json'), `${JSON.stringify({
      version,
      target,
      sourceModules:modules.map((item) => item.path),
      backgroundSourceModules:backgroundSharedPaths,
      extensionFiles:['background.js', 'background-worker.js', 'background-worker-final.js', 'content-bridge.js', 'content-bridge-final.js', 'popup.html', 'popup.css', 'popup.js']
    }, null, 2)}\n`, 'utf8')
  ]);
}
...
const diagnosticsSourceFiles = [
  'service-worker.js', 'background.js', 'background-controls.js', 'background-detach-autoexport.js',
  'background-session-health.js', 'background-streaming-export.js', 'background-discard-hardening.js',
  'background-export-resume.js', 'har-extra-info.js', 'transport.js', 'bootstrap-guard.js', 'content.js',
  'controls.js', 'export-resume-guard.js', 'export-ui-polish.js', 'export-streaming.js', 'controls-detach-autoexport.js'
];
await Promise.all([
  ...
  writeFile(resolve(diagnosticsDir, 'BUILD_INFO.json'), `${JSON.stringify({
    version:diagnosticsManifest.version,
    versionName:diagnosticsManifest.version_name || diagnosticsManifest.version,
    target:'chrome', source:'diagnostics-extension', sourceFiles:diagnosticsSourceFiles,
...
console.log(`Background worker-safe modules bundled: ${backgroundSharedPaths.length}`);
