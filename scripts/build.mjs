...
// These shared modules are function-definition safe in a worker/background
// context. Reusing them gives Chrome/Firefox the exact same index merge,
// deep-HTML parser and durable queue semantics as the Etsy-page runtime while
// IndexedDB naturally resolves to the extension origin.
const backgroundSharedPaths = [
  'src/61a-favorites-index.js',
  'src/61c-favorites-deep-parser.js',
  'src/61d-favorites-deep-queue.js'
];
...
for (const target of ['chrome', 'firefox']) {
  ...
  await Promise.all([
    ...
    writeFile(resolve(targetDir, 'BUILD_INFO.json'), `${JSON.stringify({
      ...
      backgroundSourceModules:[...backgroundSharedPaths, 'extension/background-worker.js', 'extension/background.js'],
      extensionFiles:['background.js', 'content-bridge.js', 'popup.html', 'popup.css', 'popup.js']
    }, null, 2)}\n`, 'utf8')
  ]);
}
...
console.log(`Background shared modules bundled: ${backgroundSharedPaths.length}`);
