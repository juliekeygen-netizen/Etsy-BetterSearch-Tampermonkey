...
> **Active behavior gate (2026-09-02):** [PR #79](https://github.com/juliekeygen-netizen/Etsy-BetterSearch-Tampermonkey/pull/79), branch `codex/feature-diagnostics-burst-trace`. It adds opt-in 3.2-second animation-frame pre-marker traces, 1.2-second post-marker capture, ten-frame screenshot bursts, bounded semantic mismatch markers for visible-grid/zero-count and collection-strip contradictions. Private local evidence exposed two follow-up defects in the same feature: active recorder settings were not rehydrated into the fresh panel after **Record & Reload**, and the final streaming ZIP exporter silently omitted traces/bursts even though it retained their events. This branch now restores the visual option state and exports both event families, including summary counters. Node 22 check, 566/566 tests, and all-target builds passed on the prior behavior head; final-head validation and CI are pending. It changes Diagnostics only, retains 0.2.9 identity, and contains no raw user capture data.
